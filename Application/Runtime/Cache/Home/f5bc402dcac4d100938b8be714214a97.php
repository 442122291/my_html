...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-sm-8 blog-main">
            <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="blog-post">
                <form class="form-signin" role="form" action="<?php echo U('Blog/del_blog?id',array('id'=>$vo['id']));?>" method="post">
                <h2 class="blog-post-title"><a href="<?php echo U('Content/content?id',array('id'=>$vo['id']));?>" target="_blank"><?php echo ($vo["title"]); ?></a></h2>
                    <?php if($_SESSION['level'] == 1){ ?>
                    <a href="<?php echo U('Blog/modify_blog?id',array('id'=>$vo['id']));?>" target="_blank">修改博文</a>
                    <?php } ?>
                    <p class="blog-post-meta"><?php echo ($vo["time"]); ?> by <?php echo ($vo["writer"]); ?></p>
                <p><?php echo ($vo["description"]); ?></p>
                    <?php if($_SESSION['level'] == 1){ ?>
                        <button type="submit" class="btn btn-warning">删除博文</button>
                    <?php } ?>
                </form>
